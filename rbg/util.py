...
import os
import logging
_LOGGING_LEVEL_ENVAR = 'RBG_LOGGING_LEVEL'
_LOGGING_FORMAT_ENVAR = 'RBG_LOGGING_FORMAT'
_LOGGING_FILE_PATH_ENVAR = 'RBG_LOG_FILE_PATH'
_PACKAGE_NAME_FOR_RESOURCE = 'rbg'
_PACKAGE_CONFIGFILE_ENV_VAR = 'RBG_CONFIG_FILE'
_PACKAGE_CONFIGDIR = '~/.rbg'
...
def get_config(section=None, param=None, default=None):
    ...
    if _CONFIG is None:
        ...
        if 'PEYOTL_CONFIG_FILE' in os.environ:
            _CONFIG_FN = os.environ[_PACKAGE_CONFIGFILE_ENV_VAR]
        else:
            _CONFIG_FN = os.path.expanduser(os.path.join(_PACKAGE_CONFIGDIR, 'config'))
        if not os.path.exists(_CONFIG_FN):
            from pkg_resources import Requirement, resource_filename
            pr = Requirement.parse(_PACKAGE_NAME_FOR_RESOURCE)
            try:
                _CONFIG_FN = resource_filename(os.path.join([_PACKAGE_NAME_FOR_RESOURCE, 'default.conf']))
            except:
                return default
